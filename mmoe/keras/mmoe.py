...
class MMOE(Layer):
    def __init__(self, units,
        ...
        self.expert_kernel_initializer = initializers.get(export_kernel_initializer)
    ...
    def build(self, input_shape):
        ...
        :param input_shape: Keras tensor or list/tuple of Keras tensors to reference for weight shape computations
        :return:
        """
    ...
    def call(self, inputs, **kwargs):
        """
        Methods for the forward function of the layer
        ...
        expert_outputs = K.tensorflow_backend.tf.tensordot(a=inputs, b=self.expert_kernels, axes=1)
        # Add the bias term to the expert weights if necessary
        if self.use_expert_bias:
            expert_outputs = K.bias_add(expert_outputs, bias=self.expert_bias)
        ...
        for index, gate_kernel in enumerate(self.gate_kernels):
            ...
            # add the bias term to the gate weight if necessary
            if self.use_gate_bias:
                gate_output = K.bias_add(x=gate_output, bias=self.gate_bias[index])
            gate_output = self.gate_activation(gate_output)
            print("gate_output_activate: ",gate_output.shape)
            gate_outputs.append(gate_output)
        # f^{k}(x) = sum_{i=1}^{n}(g^{k}(x)_{i} * f_{i}(x))
        for gate_output in gate_outputs:
            expanded_gate_output = K.expand_dims(gate_output, 1)
            print("expanded_gate_output", expanded_gate_output.shape)
            weighted_expert_output = expert_outputs * K.repeat_elements(expanded_gate_output, self.units, axis=1)
            print("weighted_expert_output: ",weighted_expert_output.shape)
